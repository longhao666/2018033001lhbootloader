    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->hex, &QPushButton::clicked, this, &MainWindow::slotHexClicked);
}
...
/**
 * @brief MainWindow::on_pBReadInfo_clicked
 * 读取当前ID板子的大小
 */
...
/**
 * @brief MainWindow::on_pBDownload_clicked
 *  烧入hex文件
 */
...
/**
 * @brief MainWindow::on_pBOpenHex_clicked
 * 打开文件
 */
...
void MainWindow::slotHexClicked()
{
#if 0
    qDebug() << "void MainWindow::slotHexClicked()";
#endif
    bool ok = true;
    uint16_t text = ui->lineEdit->text().toInt(&ok);
    if(!ok) {
        return ;
    }
    qDebug() << text;
    ...
    m.data[3] = 0x00;
    canSend(&m);
    ui->cBNodeid->setCurrentText(QString::number(text, 10));
}
